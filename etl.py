...
def process_log_data(spark, input_data, output_data):
    ...
    spark.udf.register("get_weekday", lambda x: datetime.datetime.fromtimestamp(x/1000).strftime("%A"))
    ...
    songplays_table = spark.sql('''select distinct get_timestamp(ts) as start_time, userId, level, song_id,\
    artist_id, sessionId, location, userAgent, get_month(ts) as month, get_year(ts) as year\
    from log_data join songs_data on log_data.artist = songs_data.artist_name and log_data.song = songs_data.title \
    and log_data.length = songs_data.duration''')
